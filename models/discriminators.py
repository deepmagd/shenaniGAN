...
class DiscriminatorStage1(Discriminator):
    ...
    def __init__(self, img_size, kernel_size, num_filters, lr, w_init, bn_init):
        ...
        self.conv1 = Conv2D(
            filters=num_filters, kernel_size=(4, 4), strides=(2, 2), padding='same', kernel_initializer=self.w_init
        )
        self.leaky_relu_1 = LeakyReLU(alpha=0.2)

        self.conv2 = Conv2D(
            filters=num_filters*2, kernel_size=(4, 4), strides=(2, 2), padding='same', kernel_initializer=self.w_init
        )
        self.bn1 = BatchNormalization(gamma_initializer=self.bn_init)
        self.leaky_relu_2 = LeakyReLU(alpha=0.2)

        self.conv3 = Conv2D(
            filters=num_filters*4, kernel_size=(4, 4), strides=(2, 2), padding='same', kernel_initializer=self.w_init
        )
        self.bn2 = BatchNormalization(gamma_initializer=self.bn_init)
        self.leaky_relu_3 = LeakyReLU(alpha=0.2)

        self.conv4 = Conv2D(
            filters=num_filters*8, kernel_size=(4, 4), strides=(2, 2), padding='same', kernel_initializer=self.w_init
        )
        ...
        self.conv5 = Conv2D(
            filters=num_filters*8, kernel_size=(1, 1), strides=(1, 1), padding="valid", kernel_initializer=self.w_init
        )

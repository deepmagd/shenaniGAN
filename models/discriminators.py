...
class DiscriminatorStage2(Discriminator):
    ...
    def build(self, input_size):
        ...
        self.conv_block_4 = ConvBlock(filters=self.d_dim*16, kernel_size=(4, 4), strides=(2, 2), padding='same',
                                      w_init=self.w_init, bn_init=self.bn_init, activation=activation
                                      )

        self.conv_block_5 = ConvBlock(filters=self.d_dim*32, kernel_size=(4, 4), strides=(2, 2), padding='same',
                                      w_init=self.w_init, bn_init=self.bn_init, activation=activation
                                      )

        self.conv_block_6 = ConvBlock(filters=self.d_dim*16, kernel_size=(4, 4), strides=(2, 2), padding='same',
                                      w_init=self.w_init, bn_init=self.bn_init, activation=activation
                                      )

        self.conv_block_7 = ConvBlock(filters=self.d_dim*8, kernel_size=(4, 4), strides=(2, 2), padding='same',
                                      w_init=self.w_init, bn_init=self.bn_init
                                      )
        ...
        self.conv_block_8 = ConvBlock(filters=self.d_dim*8, kernel_size=(1, 1), strides=(1, 1), padding='same',
                                      w_init=self.w_init, bn_init=self.bn_init, activation=activation
                                      )

        # (16, 16) == 256/16
        self.conv_2 = Conv2D(filters=1, kernel_size=(16, 16), strides=(16, 16),

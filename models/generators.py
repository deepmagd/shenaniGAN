...
class GeneratorStage1(Model):
    ...
    def loss(self, predictions_on_fake, mean, log_sigma):
        ...
        """
        kl_coeff = 1
        loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(predictions_on_fake), logits=predictions_on_fake)
        loss = tf.reduce_mean(loss) + kl_coeff * self.kl_loss(mean, log_sigma)
        return loss
        # return self.xent_loss_fn(predictions_on_fake, tf.ones_like(predictions_on_fake)) + kl_coeff * self.kl_loss(mean, log_sigma)
